...
const GUEST_POP_ARC        = 170;    // approach/recede を弧状にする下向きの反り (px、両端0・中央最大)
...
class GuestFish {
  ...
  _updatePop() {
    ...
    if (this.popPhase === 'dart') {
      ...
      const arc = Math.sin(Math.PI * e) * GUEST_POP_ARC;    // 下に弧を描いて手前へ
      ...
      this.bobPhase += 0.12;
      const bob = Math.sin(this.bobPhase) * GUEST_POP_SWIM_BOB * (1 - e);
      const arc = Math.sin(Math.PI * e) * GUEST_POP_ARC;    // 下に弧を描いて奥へ
